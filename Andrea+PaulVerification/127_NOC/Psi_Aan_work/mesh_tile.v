...
module mesh_tile #(
    ...
    wire [7:0] final_d   = boot_mode ? boot_data  : sram_wdata;
    wire       final_gwen = boot_mode ? boot_wen   : sram_wen;
    // FIX: During rst, final_cen=0 so CEN pin=1 (SRAM disabled).
    // When rst releases and boot_mode activates, final_cen goes 0->1,
    // making CEN go 1->0. This 1->0 edge sets cen_fell in the GF180
    // SRAM model, which is required before writes will be accepted.
    wire       final_cen  = rst ? 1'b0 : (boot_mode ? 1'b1 : (sram_wen | sram_ren));
