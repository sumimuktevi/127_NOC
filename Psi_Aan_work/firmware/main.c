...
/* ── Mesh geometry ──────────────────────────────────────────────────────── */
/* Physical mesh that exists in hardware */
#define PHYS_ROWS  3
#define PHYS_COLS  3

/* Logical mesh we want to use */
#define MESH_ROWS  3
#define MESH_COLS  3
...
static void seed_grid(void)
{
    ...
    /* Match currently uploaded test_mesh.py: center vertical blinker only */
    grid[4 * SIZE + 5] = 1;
    grid[5 * SIZE + 5] = 1;
    grid[6 * SIZE + 5] = 1;
}

/* ── Idle for physically present but logically unused tiles ────────────── */
...
int main(void)
{
    ...
    /* Parameterization / tile detection logic kept intact */
    if (phys_row >= PHYS_ROWS || phys_col >= PHYS_COLS) {
        idle_tile();
    }

    /* Logical placement; keep simple rectangular mapping for now */
    ...
    *debug_iter_count = 0;
    ...
    while (1) {
        ...
        /* Publish completed generation number */
        *debug_iter_count = iter;
        iter++;

        noc_signal(SIG_GEN_STABLE);

        /* Give cocotb a stable window to sample */
