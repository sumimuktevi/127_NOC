...
// ============================================================================
// mesh_tile — SERV core + GF180 SRAM + mesh_router
//
// Changes from v1:
//   Added host readback port:
//     rd_addr  [9:0]  — byte address to read (from host_spi_slave)
//     rd_req         — sample request pulse
//     rd_data  [7:0] — registered byte out (valid one sys_clk after rd_req)
//
//   The readback mux is gated: only active when NOT in boot_mode and NOT
//   being written by the CPU (rd_req is lowest priority).
// ============================================================================
...
module mesh_tile #(
    ...
    // Host readback port (read-only, lowest priority)
    ...
    // -----------------------------------------------------------------------
    // Boot mode edge detection (for CEN startup pulse)
    // -----------------------------------------------------------------------
    reg boot_mode_q;
    always @(posedge clk or posedge rst)
        if (rst) boot_mode_q <= 1'b1;
    ...
    // -----------------------------------------------------------------------
    // Address / data mux: boot > CPU > host_readback
    // -----------------------------------------------------------------------
    // The host readback port only drives the SRAM address when neither boot
    // nor CPU is actively using the read port.  We detect CPU read activity
    // via sram_ren.
    wire host_rd_active = rd_req & ~boot_mode & ~sram_ren & ~sram_wen;

    wire [9:0] final_a = boot_mode    ? boot_addr  :
                         sram_wen     ? sram_waddr  :
                         host_rd_active ? rd_addr   :
                                         sram_raddr;
    ...
    // -----------------------------------------------------------------------
    // Capture rd_data on the cycle after rd_req
    // -----------------------------------------------------------------------
    reg rd_req_q;
    always @(posedge clk) begin
        rd_req_q <= host_rd_active;
        if (rd_req_q)
            rd_data <= sram_rdata;
    end
