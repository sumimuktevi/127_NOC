...
# SRAM macros — 9x gf180mcu_fd_ip_sram__sram1024x8m8wm1 (one per mesh tile)
#
# NOTE: stripe pitch/offset/number_of_straps are carried over from the original
# 512x8 template and must be re-tuned against the actual 1024x8 macro dimensions
# (check its LEF for exact width/height). Run LibreLane and inspect the PDN
# report before sign-off.

set sram_instances {
    i_chip_core.noc_top.mesh_inst.rows[0].cols[0].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[0].cols[1].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[0].cols[2].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[1].cols[0].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[1].cols[1].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[1].cols[2].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[2].cols[0].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[2].cols[1].tile_inst.sram_inst
    i_chip_core.noc_top.mesh_inst.rows[2].cols[2].tile_inst.sram_inst
}
...
# Stripes on W/E edges — TODO: update pitch to match 1024x8 macro width
add_pdn_stripe \
    -grid sram_macros \
    -layer Metal4 \
    -width 2.36 \
...
# Additional stripes — TODO: update offset/pitch/number_of_straps to match 1024x8 height
add_pdn_stripe \
    -grid sram_macros \
    -layer Metal4 \
    -width 4.00 \
